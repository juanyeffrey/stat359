...
# Add path to Assignment_2 to get helper function
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'Assignment_2'))
from download_gensim_model import load_local_gensim_model
...
def tokenize_and_pad(sentence, model, max_len=32):
    ...
    for i, token in enumerate(tokens):
        if i >= max_len:
            break
        if token in model:
            seq[i] = model[token].astype(np.float32)
    return seq
max_seq_len = 32
X_seq = np.stack([tokenize_and_pad(sent, fasttext_model, max_len=max_seq_len) for sent in tqdm(data['sentence'], desc="Vectorizing")])
y = data['label'].values
...
optimizer = optim.Adam(model.parameters(), lr=0.0001, weight_decay=2e-4)
